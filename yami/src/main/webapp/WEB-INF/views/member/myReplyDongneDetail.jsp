...
<div class="container mt-5">
    <!-- 빵집 정보 카드 -->
    <div class="bakery-info-card">
        <div class="text-center">
            <i class="bi bi-cake"></i>
            <h4 id="bakeryTitle">빵집 정보 로딩 중...</h4>
        </div>
        <div id="bakeryInfo" class="mb-3">
            <div class="text-center py-4">
                <div class="spinner-border text-warning loading-spinner" role="status"></div>
                <div>빵집 정보를 불러오는 중...</div>
            </div>
        </div>
        <div class="d-flex justify-content-center align-items-center gap-3">
            <div class="d-flex gap-2" id="bakeryStats">
...
// 모델에서 전달받은 데이터
const targetBakeryNo = '${targetBakeryNo}';
const targetCommentNo = '${targetCommentNo}';

let currentBakeryNo = targetBakeryNo;
let currentPage = 1;
...
window.onload = function() {
    // 빵집 정보 및 댓글 로딩
    loadBakeryInfo(targetBakeryNo);
    loadBakeryComments(targetBakeryNo, 1, true);
};

/* ====== 빵집 정보 로딩 ====== */
function loadBakeryInfo(bakeryNo) {
    $.ajax({
        url: `${root}/selectBakeryInfo.dn`,
        method: 'POST',
        data: { bakeryNo: bakeryNo },
        dataType: 'json',
        success: function(res) {
            if (res.status === 'pass' && res.bakery) {
                const bakery = res.bakery;
                renderBakeryInfo(bakery);
            } else {
                document.getElementById('bakeryInfo').innerHTML = '<div class="text-danger text-center">빵집 정보를 불러오지 못했습니다.</div>';
            }
        },
        error: function() {
            document.getElementById('bakeryInfo').innerHTML = '<div class="text-danger text-center">서버 오류로 빵집 정보를 불러오지 못했습니다.</div>';
        }
    });
}

function renderBakeryInfo(bakery) {
    document.getElementById('bakeryTitle').textContent = bakery.bakeryName;
    
    const infoHtml = `
        <div class="d-flex justify-content-center align-items-center mb-2 flex-wrap gap-2">
    ...
    document.getElementById('likeCount').textContent = bakery.likeCount || 0;
    document.getElementById('dislikeCount').textContent = bakery.dislikeCount || 0;
}
...
function renderBakeryComments(comments, isFirst, hasNext) {
    ...
    // 댓글이 있을 때, '마지막 댓글입니다' 문구를 항상 마지막에 추가
    if (!hasNext) {
        html += `
    ...
    // 특정 댓글로 스크롤 (targetCommentNo가 있는 경우)
    if (targetCommentNo && isFirst) {
        setTimeout(() => {
            const targetComment = document.querySelector(`[data-comment-no="\${targetCommentNo}"]`);
            if (targetComment) {
...
function onCommentClick(commentNo) {
    ...
    if (actions) {
        ...
        lastOpenedCommentNo = actions.style.display === 'block' ? commentNo : null;
    }
}
...
function reportComment(commentNo) {
    ...
    $.ajax({
        url: `${root}/reportBakeryComment.dn`,
        method: 'POST',
